...
router.post("/get", async (req, res) => {
  ...
  const result = await query(
    "SELECT id,name,price,picture,stock FROM stores WHERE id=$1",
    [id]
  );
...
router.post("/update", async (req, res) => {
  ...
  await query(
    "UPDATE stores SET name=$1,price=$2,stock=$3 WHERE id=$4 AND user_id=$5",
...
router.post("/add_item_to_store", async (req, res) => {
  ...
  await query("INSERT INTO item_stores (item_id,store_id) VALUES ($1,$2)", [
    itemId,
    storeId
  ]);
